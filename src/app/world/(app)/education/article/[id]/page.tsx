...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      url: `https://kumami.world/world/education/article/${id}`,
      images: [ogImage],
    },
    twitter: {
...
async function ArticlePageContent({ id }: { id: string }) {
  ...
  const chapterName =
    levelNum !== null && article.chapterIndex !== undefined
...
export default async function WorldEducationArticlePage({ params }: Props) {
  const { id } = await params
  return (
    <EduEmbed>
      <Suspense
