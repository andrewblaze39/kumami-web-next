...
// Supports: paragraphs, bold (**/__), italic (*/_), underline-via-HTML, bullets (-, *)
// Mirrors the MarkdownRenderer in CRA with rawBullets=true.
...
export default function NewsArticleView({ articleId }: Props) {
  ...
  const fetchArticle = useCallback(async () => {
    try {
      ...
      const data = articleSnap.data() as Omit<NewsArticle, 'id'>
      setArticle({ id: articleSnap.id, ...data })

      // Related news (latest 6, drop current, drop drafts, take 5)
      try {
        const newsRef = collection(db, 'news')
        const relatedQuery = query(newsRef, orderBy('timestamp', 'desc'), limit(6))
        const relatedSnap = await getDocs(relatedQuery)
        const related: NewsArticle[] = relatedSnap.docs
          .map((d) => ({
            id: d.id,
            ...(d.data() as Omit<NewsArticle, 'id'>),
          }))
          .filter((item) => item.id !== articleSnap.id && item.status !== 'draft')
          .slice(0, 5)
  ...
  const articleTimestamp = article.date || article.timestamp
                  ...
                  <div className="flex-1 flex flex-col justify-between min-w-0">
                    <div>
                      <div className="flex flex-wrap gap-2 mb-1 text-xs text-gray-400">
                        ...
                        <span>
                          {formatShortTimestamp(item.date || item.timestamp)}
                        </span>
                      </div>
