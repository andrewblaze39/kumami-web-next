...
export default function MostPopularGrid() {
  ...
  const fetchGames = useCallback(async () => {
    try {
      ...
      const mostPopularGames = gamesData.filter(
        (game) =>
          hasCategory(game, 'Most Popular') &&
          game.status?.toLowerCase() !== 'draft'
      )

      setGames(mostPopularGames)
    } catch (error) {
    ...
    >
      <div className="max-w-7xl mx-auto px-8 py-12">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-4xl font-bold">Most Popular Games</h1>
          <Link
            href="/world/games"
        ...
        {games.length > 0 ? (
          ...
          <div className="flex justify-center items-center py-12">
            <p className="text-xl text-white/70">
              No most popular games available
            </p>
          </div>
        )}
      </div>
