...
export default function FreeToPlayGrid() {
  ...
  const fetchGames = useCallback(async () => {
    try {
      ...
      const freeToPlayGames = gamesData.filter(
        (game) =>
          hasCategory(game, 'Free to Play') &&
          game.status?.toLowerCase() !== 'draft'
      )

      setGames(freeToPlayGames)
    } catch (error) {
    ...
    >
      <div className="max-w-7xl mx-auto px-8 py-12">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-4xl font-bold">Free to Play Games</h1>
          <Link
            href="/world/games"
        ...
        {games.length > 0 ? (
            ...
            {games.map((game) => (
              <div key={game.id} className="space-y-2">
                <div
                  ...
                  onClick={() =>
                    router.push(
                      `/world/games/details?name=${encodeURIComponent(game.title ?? '')}`
                    )
                  }
                >
          ...
          <div className="flex justify-center items-center py-12">
            <p className="text-xl text-white/70">
              No free to play games available
            </p>
          </div>
        )}
      </div>
