...
export default function BlogArticleView({ articleId }: Props) {
  ...
  const fetchArticle = useCallback(async () => {
    try {
      ...
      const data = articleSnap.data() as Omit<BlogPost, 'id'>
      setArticle({ id: articleSnap.id, ...data })

      // Related blog posts (latest 6 by timestamp desc, drop current, drop drafts, take 5)
      try {
        ...
        const related: BlogPost[] = relatedSnap.docs
          .map((d) => ({
            id: d.id,
      ...
      setError(`Failed to load blog post: ${message}`)
    } finally {
      setLoading(false)
    }
  }, [articleId])
  ...
  const bodyOneHtml = renderMarkdown(article.content1 || article.content)
      ...
      <div className="max-w-[1000px] mx-auto px-4 md:px-8">
        ...
        >
          ...
          <div className="w-full max-w-[600px] mx-auto">
            ...
            {article.detailImage2Url && (
              <div className="my-6 w-full overflow-hidden rounded-lg bg-[#222]">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={article.detailImage2Url}
                  alt={article.title ?? 'Blog detail'}
                  ...
                  >
                    {thumb && (
                      <div className="flex-none w-full sm:w-[120px] sm:h-[90px] aspect-[4/3] sm:aspect-auto rounded-lg overflow-hidden bg-[#222]">
                        ...
                        <img
                          src={thumb}
                          alt={item.title ?? 'Related blog post'}
                          loading="lazy"
                          className="w-full h-full object-cover"
                        />
                      </div>
