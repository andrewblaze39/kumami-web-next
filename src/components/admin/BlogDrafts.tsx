...
interface Draft { id: string; title: string; category?: string; content: string; imageUrl?: string; thumbnail?: string; author?: string; level?: string; isPremium?: boolean; timestamp: { seconds: number; toDate?: () => Date } | null; [key: string]: unknown; }
...
export default function BlogDrafts() {
  ...
  const handlePublish = async (id: string) => { try { setLoading(true); await updateDoc(doc(db, 'blogs', id), { status: 'published' }); setMessage('Blog draft published!'); setDrafts((p) => p.filter((d) => d.id !== id)); } catch { setMessage('Error publishing'); } finally { setLoading(false); } };
  const handleDelete = async (id: string) => { if (!window.confirm('Delete this draft?')) return; try { setLoading(true); await deleteDoc(doc(db, 'blogs', id)); setMessage('Draft deleted!'); setDrafts((p) => p.filter((d) => d.id !== id)); } catch { setMessage('Error deleting'); } finally { setLoading(false); } };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <h2 className="text-2xl font-bold text-gray-800 mb-6">Blog Drafts</h2>
          ...
          {drafts.map((draft) => (
            <div key={draft.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <div className="p-4">
                ...
                <p className="text-gray-600 mb-4 line-clamp-2">{truncateContent(draft.content || '')}</p>
                ...
                <div className="flex space-x-2 mt-4">
                  <button className="flex-1 py-2 px-3 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors" onClick={() => router.push('/admin/edit-blog?id=' + draft.id)}>Edit</button>
