...
export default function PartnerDrafts() {
  ...
  const fetchDrafts = async () => {
    try {
      const q = query(collection(db, 'partner_articles'), where('status', '==', 'draft'));
  ...
  const handlePublish = async (id: string) => { try { setLoading(true); await updateDoc(doc(db, 'partner_articles', id), { status: 'published' }); setMessage('Partner draft published!'); setDrafts((p) => p.filter((d) => d.id !== id)); } catch { setMessage('Error publishing'); } finally { setLoading(false); } };
  const handleDelete = async (id: string) => { if (!window.confirm('Delete this draft?')) return; try { setLoading(true); await deleteDoc(doc(db, 'partner_articles', id)); setMessage('Draft deleted!'); setDrafts((p) => p.filter((d) => d.id !== id)); } catch { setMessage('Error deleting'); } finally { setLoading(false); } };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <h2 className="text-2xl font-bold text-gray-800 mb-6">Partner Drafts</h2>
          ...
          {drafts.map((draft) => (
            <div key={draft.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <div className="p-4">
                ...
                <p className="text-gray-600 mb-4 line-clamp-2">{truncateContent(draft.description || '')}</p>
                ...
                <div className="flex space-x-2 mt-4">
                  <button className="flex-1 py-2 px-3 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors" onClick={() => router.push('/admin/edit-partner-articles?id=' + draft.id)}>Edit</button>
