...
export default function ManageHomepagePartners() {
  ...
  useEffect(() => { const fetch = async () => { try { const snap = await getDocs(query(collection(db, 'homepage_partners'))); const list = snap.docs.map((d) => ({ id: d.id, ...d.data() } as Partner)).sort((a, b) => (a.name || '').localeCompare(b.name || '')); setPartners(list); setFilteredPartners(list); } catch { setError('Failed to load'); } finally { setLoading(false); } }; fetch(); }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) { await updateDoc(doc(db, 'homepage_partners', editingId), { name: formData.name, ...(logoUrl ? { logoUrl } : {}), link: formData.link, updatedAt: serverTimestamp() }); }
      else { await addDoc(collection(db, 'homepage_partners'), { name: formData.name, logoUrl, link: formData.link, createdAt: serverTimestamp() }); }
      resetForm();
      const snap = await getDocs(query(collection(db, 'homepage_partners'))); const list = snap.docs.map((d) => ({ id: d.id, ...d.data() } as Partner)).sort((a, b) => (a.name || '').localeCompare(b.name || '')); setPartners(list); setFilteredPartners(list);
    } catch (err: unknown) { setError(err instanceof Error ? err.message : 'Error'); }
  };

  const handleEdit = (p: Partner) => { setFormData({ name: p.name || '', link: p.link || '' }); setLogoPreview(p.logoUrl || ''); setEditingId(p.id); };
  const handleDelete = async (id: string) => { if (!window.confirm('Delete?')) return; try { await deleteDoc(doc(db, 'homepage_partners', id)); setPartners((p) => p.filter((x) => x.id !== id)); setFilteredPartners((p) => p.filter((x) => x.id !== id)); } catch { setError('Failed to delete'); } };

  if (!isSuperOrAdmin) return <div className="p-8"><p className="text-gray-600">No permission.</p></div>;

  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Manage Homepage Partners</h2>
