...
/**
 * CoursesHome — Andrew's original /education/[phase] course page ("My Courses"
 * view from the main branch), extracted into an embed component so it renders
 * inside the world shell at /world/education?tab=courses.
 *
 * The level comes from the `?level=` query param (1–5, default 1) instead of
 * the old route segment. Chapter/article links point at the world-shell
 * article route; the next-level preview switches the query param.
 */
...
export default function CoursesHome() {
  ...
  const levelNum = PHASES.some(p => p.n === rawLevel) ? rawLevel : 1
  ...
  // Reset accordion/tab state when switching levels
  useEffect(() => {
    setActiveTab('overview')
    setOpenChapters(new Set([0]))
  }, [levelNum])
  ...
  const firstChapter = chapters.find(a => a.comingSoon !== true)
  const startHref = firstChapter ? `/world/education/article/${firstChapter.id}` : null
    ...
    <div className="edu-content">
      ...
      {activeTab === 'overview' && (
        <div>
          ...
          {chapters.map((article) => {
            ...
            const chapterHref = `/world/education/article/${article.id}`
      ...
      {activeTab === 'overview' && levelData.n < 5 && (
        ...
        >
          ...
          <Link
            href={`/world/education?tab=courses&level=${levelData.n + 1}`}
